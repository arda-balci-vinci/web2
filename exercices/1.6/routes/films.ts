...
if (!title || !director){
  return res.status(400).json({ error: "Missing title or director" });
} 
if (!Number.isFinite(duration) || duration <= 0) {
    return res.status(400).json({ error: "Wrong duration" });
}
if (budget !== undefined && (!Number.isFinite(budget) || budget <= 0)){
     return res.status(400).json({ error: "Wrong budget" });
}
...
router.delete('/:id',(req,res)=> {
  ...
  const index = defaultFilms.findIndex((defaultFilms) => defaultFilms.id === id);

  if(index ===-1){
    return res.status(400).json("film not found");
  }
